...
import io.github.justincodinguk.c_hat_server_v2.core.email
        ...
        if(frame is Frame.Text) {
            ...
            if(deserializedResponse.event == Event.REGISTER) {
                ...
                if(verificationCode == (deserializedResponse as VerificationCodeReceivedResponse).code) {
                    if(newUser != null) {
                        eventBus.fireEvent(io.github.justincodinguk.c_hat_server_v2.eventbus.Event.ON_REGISTER_COMPLETE)
                        dao.insert(newUser)
                        send(Json.encodeToString(RegisterSuccessResponse()))
                        close(CloseReason(CloseReason.Codes.NORMAL, "Registration Successful"))
                    }
                } else {
                    eventBus.fireEvent(io.github.justincodinguk.c_hat_server_v2.eventbus.Event.ON_REGISTER_FAILED)
